...
export async function getQAParameterById(
  id: string
): Promise<QAParameterDocument | null> {
  try {
...
export async function updateQAParameter(
  parameterId: string,
  updateData: Partial<QAParameterDocument>
): Promise<QAParameterDocument | null> {
  try {
...
export async function searchQAParameters(
  searchTerm: string
): Promise<QAParameterDocument[]> {
  try {
...
export async function toggleQAParameterStatus(
  parameterId: string
): Promise<QAParameterDocument | null> {
  try {
    const currentParameter = (await QAParameter.findById(
      parameterId
    ).lean()) as IQAParameter | null;
    if (!currentParameter) {
      return null;
    }
...
export async function linkQAParameterToSOP(
  parameterId: string,
  sopId: string
): Promise<boolean> {
  try {
    const result = await QAParameter.findByIdAndUpdate(parameterId, {
...
export async function unlinkQAParameterFromSOP(
  parameterId: string
): Promise<boolean> {
  try {
    const result = await QAParameter.findByIdAndUpdate(parameterId, {
...
export function createSubParameter(
  name: string,
  weight: number,
...
export async function duplicateQAParameter(
  parameterId: string,
  newName: string,
  createdBy: string
): Promise<QAParameterDocument | null> {
  try {
    ...
    const duplicatedParameters = originalParameter.parameters.map((param) => ({
      ...
      subParameters: param.subParameters.map((subParam) => ({
        ...subParam,
        id: `sub_param_${Date.now()}_${Math.random()
          .toString(36)
          .substr(2, 9)}`,
      })),
    }));
