...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      ...
      if (category) {
        sops = sops.filter(
          (sop) => sop.category.toLowerCase() === category.toLowerCase()
        );
      }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Add required createdBy field - will be provided by frontend
    const sopDataWithRequiredFields = {
      ...sopData,
      createdBy: "system", // Default creator for all SOPs
    };

    const newSOP = await createSOP(sopDataWithRequiredFields);

    return NextResponse.json({ success: true, data: newSOP }, { status: 201 });
  } catch (error: any) {
