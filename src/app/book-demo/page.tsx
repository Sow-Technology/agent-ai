...
export default function BookDemoPage() {
  ...
  // Set plan from URL when component mounts
  useEffect(() => {
    if (planFromUrl) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setIsSuccess(true);
        setFormData({ name: "", email: "", company: "", message: "", plan: planFromUrl || "" });
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <div>
                ...
                {planFromUrl && (
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/30 mb-6 ml-3">
                    <Package className="w-4 h-4 text-emerald-500" />
                    <span className="text-sm font-semibold text-emerald-400">{planFromUrl} Plan Selected</span>
                  </div>
                )}
