...
const subParameterSchema = z.object({
  ...
  name: z
    .string()
    .min(1, "Sub-parameter name cannot be empty.")
    .max(150, "Sub-parameter name is too long."),
  weight: z.preprocess(
    (val) =>
...
const parameterSchema = z.object({
  ...
  name: z
    .string()
    .min(1, "Parameter group name cannot be empty.")
    .max(100, "Group name is too long."),
  subParameters: z
    .array(subParameterSchema)
    .min(1, "Each group must have at least one sub-parameter."),
});
...
const campaignSchema = z
  .object({
    ...
    parameters: z
      ...
      .min(1, "A campaign must have at least one parameter group."),
  ...
  .refine(
    ...
    {
      message:
        "The sum of all sub-parameter weights across all groups must be exactly 100.",
      path: ["parameters"],
    }
  );
...
export default function ParameterManagementPage() {
  ...
  const [campaignToDelete, setCampaignToDelete] = useState<QAParameter | null>(
    null
  );
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingCampaign, setEditingCampaign] = useState<QAParameter | null>(
    null
  );
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const parameters = parametersData.success
          ? parametersData.data
          : parametersData;
      ...
      parameters: [
        {
          ...
          subParameters: [
            {
              id: `sub_${Date.now()}_1`,
              name: "Used standard greeting",
              weight: 50,
              type: "Non-Fatal",
            },
            {
              id: `sub_${Date.now()}_2`,
              name: "Was professional",
              weight: 50,
              type: "Non-Fatal",
            },
          ],
    ...
    try {
      if (editingCampaign) {
        const response = await fetch(
          `/api/qa-parameters/${editingCampaign.id}`,
          {
            method: "PUT",
        ...
        const updatedCampaignData = updatedCampaign.success
          ? updatedCampaign.data
          : updatedCampaign;
        setCampaigns(
          campaigns.map((c) =>
            c.id === editingCampaign.id
              ? convertQAParameterDocumentToQAParameter(updatedCampaignData)
              : c
          )
        );
        toast({
          title: "Campaign Updated",
          description: `Campaign "${submissionData.name}" has been updated.`,
        });
      } else {
        ...
        const newCampaignData = newCampaign.success
          ? newCampaign.data
          : newCampaign;
        setCampaigns([
          convertQAParameterDocumentToQAParameter(newCampaignData),
          ...campaigns,
        ]);
        toast({
          title: "Campaign Created",
          description: `Campaign "${submissionData.name}" has been created.`,
        });
      }
      closeForm();
    } catch (error) {
    ...
    <div className="space-y-6">
      <Suspense fallback={<div>Loading...</div>}>
        <Card className="shadow-lg">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
              <div className="mb-4 sm:mb-0">
                <CardTitle className="text-3xl font-bold flex items-center">
                  <ListChecks className="mr-3 h-8 w-8 text-primary" /> QA Audit
                  Parameter Campaigns
                </CardTitle>
                <CardDescription>
                  Manage hierarchical QA campaigns. Create parameter groups, add
                  weighted sub-parameters, and mark items as Fatal or ZTP.
                </CardDescription>
              </div>
              <Button
          ...
          <CardContent>
            <div className="mb-6">
              <Label htmlFor="search-params" className="sr-only">
                Search Campaigns
              </Label>
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)}>
                    <CardHeader>
                      ...
                      <CardDescription>
                        {editingCampaign
                          ? `Update the details for campaign "${editingCampaign.name}".`
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormControl>
                                <Input
                                  placeholder="e.g., Standard Product Audit"
                                  {...field}
                                />
                              </FormControl>
                              <FormMessage />
                      ...
                      <div className="space-y-4">
                        ...
                        {parameterFields.map((groupField, groupIndex) => (
                          <Collapsible
                            key={groupField.id}
                            defaultOpen
                            className="p-4 border rounded-lg space-y-4 bg-card/50"
                          >
                            <div className="flex items-center justify-between">
                              <CollapsibleTrigger asChild>
                                <div className="flex items-center gap-2 cursor-pointer">
                                  <FormField
                                    control={form.control}
                                    name={`parameters.${groupIndex}.name`}
                                    render={({ field }) => (
                              ...
                              <Button
                                ...
                                variant="destructive"
                                size="sm"
                                onClick={() => removeParameter(groupIndex)}
                              >
                                Remove Group
                              </Button>
                            </div>
                        ...
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          onClick={handleAddNewParameterGroup}
                        >
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredCampaigns.map((c) => {
                      const linkedSop = availableSops.find(
                        (s) => s.id === c.linkedSopId
                      );
                      return (
                        <TableRow key={c.id}>
                          ...
                          <TableCell>
                            {linkedSop
                              ? `${linkedSop.title} (v${linkedSop.version})`
                              : "N/A"}
                          </TableCell>
                          <TableCell>
                            <span
                          ...
                          <TableCell>
                            {format(new Date(c.lastModified), "PPp")}
                          </TableCell>
            ...
            {filteredCampaigns.length === 0 && !isFormOpen && (
              <p className="text-center text-muted-foreground py-8">
                {searchTerm
                  ...
                  : 'No campaigns defined yet. Click "Add New Campaign" to get started.'}
              </p>
            )}
          </CardContent>
        </Card>
  ...
  return (
    <div className="space-y-3 pl-4 border-l-2 border-primary/20">
      ...
      {fields.map((subField, subIndex) => (
        <div
          key={subField.id}
          className="grid grid-cols-[1fr,100px,140px,auto] gap-2 items-start"
        >
          <FormField
            control={control}
            name={`parameters.${groupIndex}.subParameters.${subIndex}.name`}
            render={({ field }) => (
              <FormItem>
                <FormLabel className="sr-only">Name</FormLabel>
                <FormControl>
                  <Input placeholder="Sub-parameter name" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            control={control}
            name={`parameters.${groupIndex}.subParameters.${subIndex}.weight`}
            render={({ field }) => (
              <FormItem>
          ...
          <FormField
            control={control}
            name={`parameters.${groupIndex}.subParameters.${subIndex}.type`}
            render={({ field }) => (
              <FormItem>
