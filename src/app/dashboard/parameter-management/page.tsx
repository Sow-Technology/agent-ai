...
export default function ParameterManagementPage() {
 ...
 if (storedParameters) {
 try {
 setCampaigns(JSON.parse(storedParameters));
 } catch (e) {
 console.error("Failed to parse parameters from localStorage", e);
 setCampaigns([]);
 }
      }
 ...
 setAvailableSops([]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem(LOCAL_STORAGE_KEY_PARAMETERS, JSON.stringify(campaigns));
    }
  }, [campaigns]);
 ...
 c.id === editingCampaign.id ? { ...editingCampaign, ...submissionData, lastModified: new Date().toISOString() } : c
      );
