  ...
  const onSubmit = async (data: UserFormValues) => {
    try {
      if (editingUser) {
        ...
        if (!data.password) { // If password is not changed, don't update it
        ...
        if (response.ok && result.success) {
          ...
          toast({ title: 'User Updated', description: `User "${data.fullName}" has been updated.` });
        } else {
          toast({ title: 'Update Failed', description: result.error || 'Could not update user.', variant: 'destructive' });
        }
      } else {
        ...
        if (response.ok && result.success) {
          ...
          toast({ title: 'User Created', description: `User "${data.fullName}" has been created.` });
        } else {
          toast({ title: 'Creation Failed', description: result.error || `A user with username "${data.username}" may already exist.`, variant: 'destructive' });
        }
      }
      closeForm();
    } catch (error) {
  ...
  const handleDeleteUser = async (userId: string) => {
    try {
      ...
      if (response.ok && result.success) {
        ...
        toast({ title: 'Deletion Failed', description: result.error || 'Could not delete user.', variant: 'destructive'});
      }
    } catch (error) {
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid md:grid-cols-2 gap-4">
                      ...
                      <FormField control={form.control} name="username" render={({ field }) => (
                          <FormItem><FormLabel>Username</FormLabel><FormControl><Input placeholder="johndoe" {...field} disabled={!!editingUser} /></FormControl><FormMessage /></FormItem>
                      )}/>
                       ...
                       <FormField control={form.control} name="password" render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                                <Input 
                                    type="password" 
                                    placeholder={editingUser ? "Leave blank to keep current password" : "Enter password (min. 6 chars)"} 
                                    {...field} 
                                />
                            </FormControl>
                            <FormMessage />
                ...
                {users.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="text-right space-x-2">
                      <Button variant="outline" size="sm" onClick={() => openFormForEdit(user)}><Edit className="h-4 w-4" /></Button>
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                           <Button variant="destructive" size="sm"><Trash2 className="h-4 w-4" /></Button>
                        </AlertDialogTrigger>
                        <AlertDialogContent>
                            <AlertDialogHeader><AlertDialogTitle>Are you sure?</AlertDialogTitle><AlertDialogDescription>This will permanently delete the user <strong className="px-1">{user.fullName}</strong>.</AlertDialogDescription></AlertDialogHeader>
