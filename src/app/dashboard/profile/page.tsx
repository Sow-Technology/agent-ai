...
import type { User } from "@/lib/authService"; // Assuming User type is exported from authService or a shared types file
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (isClient) {
      ...
      if (userDetails) {
        finalFullName = userDetails.fullName || DEFAULT_FULL_NAME;
        finalEmail = userDetails.email || DEFAULT_EMAIL;
        finalUsername = userDetails.username || DEFAULT_USERNAME;
        finalRole = userDetails.role || DEFAULT_ROLE;
      } else {
        // If no details found (e.g. first load, or cleared storage), save defaults
        try {
          localStorage.setItem(
            LOCAL_STORAGE_LOGGED_IN_USER_DETAILS_KEY,
            JSON.stringify({
          ...
          console.error(
            "Failed to set default user details in localStorage",
            error
          );
        }
      }
  ...
  const handleSaveProfile = () => {
    if (isClient) {
      try {
        const currentDetailsRaw = localStorage.getItem(
          LOCAL_STORAGE_LOGGED_IN_USER_DETAILS_KEY
        );
        ...
        const updatedDetails: User = {
          ...
          role: currentDetails.role || profileRole, // Role is not editable here, preserve it
            ...
            <div>
              ...
              <div className="space-y-3 p-4 border rounded-md bg-muted/30 shadow-sm">
                ...
                <p className="text-xs text-muted-foreground">
                  Treat your API keys like passwords. Keep them secret and
                  secure. This is a mock API key for demonstration.
                </p>
              </div>
            </div>
