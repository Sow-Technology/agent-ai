...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter, DialogTrigger, DialogDescription as DialogDesc } from "@/components/ui/dialog";
...
const INITIAL_API_KEY = `sk_live_mock_${Array(24).fill(0).map(() => Math.random().toString(36).charAt(2)).join('')}`;
...
export default function ProfilePage() {
  ...
  const currentDisplayedAvatar = customAvatarSrc || placeholderAvatars[currentAvatarIndex];

  const handleChangeAvatar = () => {
    setCustomAvatarSrc(null);
    setCurrentAvatarIndex((prevIndex) => (prevIndex + 1) % placeholderAvatars.length);
  };
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      toast({
        ...
        description: "Your new avatar is now previewing. Click 'Save Profile' to apply (mock save for avatar).",
      });
    }
  };
  ...
  const handleDiscardChanges = () => {
    ...
    toast({
      ...
      description: "Your profile information has been reverted to the last saved state.",
    });
  };

  const generateNewApiKey = () => {
    const newKey = `sk_live_mock_${Array(24).fill(0).map(() => Math.random().toString(36).charAt(2)).join('')}`;
    setApiKey(newKey);
    toast({
        ...
        <Card className="shadow-lg">
          ...
          <CardContent className="space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2"><div className="h-4 w-20 bg-muted rounded mb-1"></div><div className="h-10 bg-muted rounded"></div></div>
                <div className="space-y-2"><div className="h-4 w-20 bg-muted rounded mb-1"></div><div className="h-10 bg-muted rounded"></div></div>
                <div className="space-y-2"><div className="h-4 w-20 bg-muted rounded mb-1"></div><div className="h-10 bg-muted rounded"></div></div>
                <div className="space-y-2"><div className="h-4 w-20 bg-muted rounded mb-1"></div><div className="h-10 bg-muted rounded"></div></div>
            </div>
        ...
        <CardContent className="space-y-8">
          <div className="flex items-center space-x-6">
            <Avatar className="h-24 w-24">
              <AvatarImage src={currentDisplayedAvatar} alt="User Avatar" data-ai-hint="user portrait"/>
              <AvatarFallback>{profileFullName.substring(0,2).toUpperCase()}</AvatarFallback>
            </Avatar>
            <div className="space-y-2">
            ...
            <div className="space-y-4">
                <Dialog open={isPasswordDialogOpen} onOpenChange={setIsPasswordDialogOpen}>
                  ...
                  <DialogContent className="sm:max-w-[425px]">
                    ...
                    <form onSubmit={handlePasswordChangeSubmit} className="space-y-4 py-4">
                      <div>
          ...
          <div>
            ...
            <div className="space-y-3 p-4 border rounded-md bg-muted/30 shadow-sm">
              ...
              <p className="text-xs text-muted-foreground">
                Treat your API keys like passwords. Keep them secret and secure. This is a mock API key for demonstration.
              </p>
            </div>
          </div>
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button className="bg-accent hover:bg-accent/90 text-accent-foreground" onClick={handleSaveProfile}>Save Profile</Button>
          </div>
        </CardContent>
      </Card>
       <Card className="shadow-lg">
